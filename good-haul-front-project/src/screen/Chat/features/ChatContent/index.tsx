...
export default function ChatContent({ route, navigation }) {
  ...
  //应该要在这里设置，否则下一次进入房间应该就不能连接到服务器了！
  useFocusEffect(
    () => {
      if (route.params.chatContent && !roomId) {
        ...
        navigation.setOptions({
          title: "司机 " + chatContent.driver.name,
          headerRight: () => (
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <TouchableOpacity style={{ alignItems: "center", marginRight: 10 }} onPress={handleDriverCallPress(chatContent.driver.phone)}>
                <FontAwesome name="phone" color={"#737373"} size={20} />
                {/* <Text style={{ fontSize: 13 }}>电话</Text> */}
              </TouchableOpacity>
  ...
  useEffect(() => {
    setChatUser({
      _id: userInfo.id,
      name: userInfo?.profile?.name ? userInfo?.profile?.name :
        userInfo.phone.substring(0, 3) + '****' + userInfo.phone.substring(7),
      avatar: userInfo?.profile?.avatar ? userInfo?.profile?.avatar : "https://aronimage.oss-cn-hangzhou.aliyuncs.com/img/huoche.png",
    })
  }, [userInfo])
