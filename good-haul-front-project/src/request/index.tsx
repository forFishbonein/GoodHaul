...
import { useMyNavigation } from "../utils/hooks/navigationHook"
...
const useAxios = () => {
  ...
  const { navigationRef } = useMyNavigation();
  let toast = useToast();
  ...
  service.interceptors.response.use(
    ...
    async (error: any) => {
      ...
      if (isEnglish(res.data.message) || !res.data.message) { //如果res.data里面本来就有中文的message，那么就没必要设置了！
        ...
        return Promise.reject(error); //上面的都已经处理完毕了，不需要再向下传递，否则会给警告说没有进行处理！
      }
